   ...
   // loadAllGenres();
    ...
    function consoleLogAudioFeatures(track, one) {
        ...
        currentApi.getAudioFeatures(track.id).then(function (data) {
            if (one) {
                console.log("I GOT CALLED FOR BEYONCE");
                //track_one = track;
    ...
    function update_track(track_obj, one) {
        ...
        if (one) {
            ...
            document.getElementById('track-album').innerHTML = 'Album:  <b><i>' + track_obj[track_obj.length-1].album_title + '</i></b>';
